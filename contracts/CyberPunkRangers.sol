...
interface ICyberPunkRangers is IERC721Enumerable {
    function isMintedBeforeReveal(uint256 index) external view returns (bool);
}

contract CyberPunkRangers is ICyberPunkRangers, ERC721, Ownable {
    using SafeMath for uint256;

    string public constant CYBERPUNKRANGERS_PROVENANCE = "df760c771ad006eace0d705383b74158967e78c6e980b35f670249b5822c42e1";

    uint256 public constant SALE_START_TIMESTAMP = 1629380700; // 3 September

    // Time after which CyberPunkRangers are randomized and allotted 3 September + 14 days
    uint256 public constant REVEAL_TIMESTAMP = SALE_START_TIMESTAMP + (86400 * 14);

    uint256 public constant NAME_CHANGE_PRICE = 365 * (10 ** 18);

    uint256 public constant DESCRIPTION_PER_CHAR_PRICE = 1 * (10 ** 18);

    uint256 public constant MAX_NFT_SUPPLY = 13684;
    ...
    // Mapping from token ID to whether the CyberPunkRanger was minted before reveal
    ...
    // Events
    event NameChange (uint256 indexed rangerIndex, string newName);
    event DescriptionChange (uint256 indexed rangerIndex, string newDescription);

    /**
     * @dev Initializes the contract by setting a `name` and a `symbol` to the token collection.
     */
    constructor(string memory name, string memory symbol, address nctAddress, string memory baseURI) ERC721(name, symbol) {
    ...
    /**
     * @dev Gets current CyberPunkRanger Price
     */
    function getNFTPrice() public view returns (uint256) {
        ...
        if (currentSupply >= 13685) {
            return 66000000000000000000; // 13685 - 13690 66 ETH
        } else if (currentSupply >= 13666) {
            return 9000000000000000000; // 13666 - 13684 9 ETH
        } else if (currentSupply >= 13000) {
            return 3000000000000000000; // 13000  - 13665 3 ETH
        } else if (currentSupply >= 10000) {
            return 1600000000000000000; // 10000 - 12999 1.6 ETH
        } else if (currentSupply >= 7000) {
            return 999000000000000000; // 7000 - 9999 0.999 ETH
        } else if (currentSupply >= 4000) {
            return 600000000000000000; // 4000 - 6999 0.6 ETH
        } else if (currentSupply >= 2000) {
            return 300000000000000000; // 2000 - 3999 0.3 ETH
        } else {
    ...
    */
    ...
    /**
     * @dev Changes the name for CyberPunkRanger tokenId
     */
    ...
    function changeDescription(uint256 tokenId, string memory newDescription) public {
        ...
        IERC20(_nctAddress).burn(descriptionPriceChange);
    ...
    function validateName(string memory str) public pure returns (bool){
        return validateDescription(str, 25);
    }

    /**
     * @dev Check if the description string is valid (Alphanumeric and spaces without leading or trailing space)
     */
    function validateDescription(string memory str, uint length) public pure returns (bool){
    ...
    /**
     * @dev Get the description string count
     */
    function getDescriptionCount(string memory str) public pure returns (uint){
        bytes memory b = bytes(str);
        return b.length;
    }
